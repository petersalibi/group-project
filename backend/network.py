        ...
        match type:
            ...
            case TrainingDataType.PENGUINS:
                ...
                url = str(
                    Path(__file__)
                    .resolve()
                    .parent.joinpath("data", "training", "penguins.csv")
                )
            ...
            case TrainingDataType.PURPLECOLOURS:
                ...
                url = str(
                    Path(__file__)
                    .resolve()
                    .parent.joinpath("data", "training", "purple_colours.csv")
                )
                df = pd.read_csv(url).dropna()
...
class NetworkParams:
    def __init__(self, activation: nn.Module = nn.Tanh(), depth=2, width=10):
...
class Model(nn.Module):
    def __init__(self, params: NetworkParams, inputs, outputs):
        ...
        if params.depth == 1:
            layers = nn.Sequential(nn.Linear(inputs, outputs))
            self.net = layers
        else:
...
def generate_purple_colours_data(n_samples=100, zero_centered=False):
    ...
    X_purple = torch.rand(n_samples // 2, 3) * torch.tensor(
        [0.5, 0.5, 0.5]
