...
import { initScene, cleanupScene, handleResize, createLandscapeMesh } from '../utils/threejs-utils';
...
const Optimiser3DVisualiser = forwardRef<OptimiserVisualiserHandle, Props>(({ optimizer, learningRate, curveRef, forcesRef }, ref) => {
  ...
  useEffect(() => {
    if (propsRef.current.optimizer !== optimizer) optStateRef.current = { m1: 0, m2: 0, v1: 0, v2: 0, t: 0 };
  ...
  const getInterpolatedLoss = (x: number, z: number) => {
    ...
    const c0 = Math.floor(col); const c1 = Math.min(c0 + 1, N - 1);
    const r0 = Math.floor(row); const r1 = Math.min(r0 + 1, M - 1);
    ...
    const loadLandscape = async () => {
      ...
      try {
        const payload = {
          ...
          method: 'RANDOMDIRS', data: 'SINREGRESSION', loss: 'MSELoss', rawdata: null, args: null
        };
        ...
        const marker = new THREE.Mesh(new THREE.SphereGeometry(0.04, 16, 16), new THREE.MeshBasicMaterial({ color: '#ffffff' }));
        scene.add(marker);
        const line = new THREE.Line(new THREE.BufferGeometry(), new THREE.LineBasicMaterial({ color: 0xffffff, linewidth: 2 }));
        scene.add(line);
        ...
        const stepArrow = new THREE.ArrowHelper(new THREE.Vector3(1,0,0), new THREE.Vector3(), 0, 0x10b981, 0.05, 0.03); 
        scene.add(gradArrow); scene.add(momArrow); scene.add(stepArrow);

        // Initialize Position
        const l0 = getInterpolatedLoss(posRef.current.x, posRef.current.z);
        const y0 = getWorldY(l0);
        pathRef.current = [{ x: posRef.current.x, z: posRef.current.z, y: y0, loss: l0 }];
        marker.position.set(posRef.current.x, y0 + 0.04, posRef.current.z);

        sceneRef.current = { scene, camera, renderer, controls, mesh, marker, line, gradArrow, momArrow, stepArrow };
        setStats({ w1: posRef.current.x, w2: posRef.current.z, loss: l0, steps: 0 });
        drawCurve();
  ...
  const stepOptimizer = () => {
    ...
    const vecPath = pathRef.current.map(p => new THREE.Vector3(p.x, p.y + 0.02, p.z));
    ...
    const updateArrow = (arrow: any, ax: number, az: number, scale: number) => {
  ...
  const drawCurve = () => {
    ...
    ctx.beginPath();
    path.forEach((p, i) => { 
        i === 0 ? ctx.moveTo(mapX(i), mapY(p.loss)) : ctx.lineTo(mapX(i), mapY(p.loss)); 
    });
    ctx.stroke();
    ...
    ctx.fillStyle = '#fff';
    ctx.beginPath(); ctx.arc(mapX(path.length - 1), mapY(lastP.loss), 4, 0, Math.PI * 2); ctx.fill();
  };

  const drawForces = (g1: number, g2: number, m1: number, m2: number, s1: number, s2: number) => {
  ...
  useEffect(() => {
    let raf: number;
    const loop = () => { stepOptimizer(); raf = requestAnimationFrame(loop); };
  ...
  useImperativeHandle(ref, () => ({
    step: () => stepOptimizer(),
    toggleRun: () => { setIsRunning(!isRunning); return !isRunning; },
    reset: () => {
      setIsRunning(false);
      optStateRef.current = { m1: 0, m2: 0, v1: 0, v2: 0, t: 0 };
  ...
  return (
    <View style={{ flex: 1, width: '100%', position: 'relative', overflow: 'hidden' }}>
        ...
        {/* GUI OVERLAY */}
        <View style={{ position: 'absolute', top: 16, left: 16, padding: 12, backgroundColor: 'rgba(0,0,0,0.8)', borderRadius: 8, borderWidth: 1, borderColor: theme.colors.border }}>
            <Text style={{ fontSize: 10, color: theme.colors.mutedForeground, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                Click landscape to teleport
            </Text>
            <View style={{ flexDirection: 'row', gap: 16 }}>
                <View>
                    <Text style={{ fontSize: 9, color: theme.colors.mutedForeground }}>PROJECTION X</Text>
                    <Text style={{ fontSize: 14, color: '#fff', fontFamily: 'monospace', fontWeight: 'bold' }}>{stats.w1.toFixed(3)}</Text>
                </View>
                <View>
                    <Text style={{ fontSize: 9, color: theme.colors.mutedForeground }}>PROJECTION Y</Text>
                    <Text style={{ fontSize: 14, color: '#fff', fontFamily: 'monospace', fontWeight: 'bold' }}>{stats.w2.toFixed(3)}</Text>
                </View>
            </View>
            <View style={{ height: 1, backgroundColor: 'rgba(255,255,255,0.1)', marginVertical: 8 }} />
            <View style={{ flexDirection: 'row', gap: 16 }}>
                <View>
                    <Text style={{ fontSize: 9, color: theme.colors.mutedForeground }}>MSE LOSS</Text>
                    <Text style={{ fontSize: 14, color: '#ef4444', fontFamily: 'monospace', fontWeight: 'bold' }}>{stats.loss.toFixed(4)}</Text>
                </View>
                <View>
                    <Text style={{ fontSize: 9, color: theme.colors.mutedForeground }}>STEPS</Text>
                    <Text style={{ fontSize: 14, color: theme.colors.frenchBlue, fontFamily: 'monospace', fontWeight: 'bold' }}>{stats.steps}</Text>
                </View>
            </View>
