...
import { Activity, Layers, ZoomIn, ZoomOut, RefreshCw, Database } from "lucide-react-native";
...
export function ActivationLesson({ onTaskUpdate }: any) {
  ...
  const [activation, setActivation] = useState<'Linear' | 'ReLU' | 'Tanh'>('Linear');
  ...
  useEffect(() => {
    ...
    const fetchLandscape = async () => {
      ...
      try {
        ...
        if (activation === 'Linear' && depth >= 3 && width >= 3 && !hasSeenDeepLinear) setHasSeenDeepLinear(true);
        if (hasSeenDeepLinear && activation === 'ReLU' && !hasSeenDeepReLU) setHasSeenDeepReLU(true);
        if (hasSeenDeepReLU && activation === 'Tanh' && !hasSeenDeepTanh) setHasSeenDeepTanh(true);
  ...
  useEffect(() => {
    if (onTaskUpdate) {
      if (hasSeenDeepTanh) {
        ...
        if (activation !== 'Linear') {
          onTaskUpdate(false, "Wait! Go back to 'Linear' first. Increase DEPTH and WIDTH to at least 3 to prove that deep linear networks are still just flat bowls.", true);
        } else {
          onTaskUpdate(false, "Step 1: Prove that linear networks can't fold space. Increase DEPTH and WIDTH to at least 3.");
        }
        
      } else if (!hasSeenDeepReLU) {
        onTaskUpdate(false, "Step 2: Notice it's STILL a flat bowl! Now, switch to 'ReLU' to see how non-linearity instantly shatters the landscape.");
      } else {
        onTaskUpdate(false, "Step 3: ReLU creates sharp creases. Now switch to 'Tanh' to see how a smooth activation bends the landscape into waves.");
      }
    }
  }, [activation, depth, width, hasSeenDeepLinear, hasSeenDeepReLU, hasSeenDeepTanh]);
      ...
      <View style={styles.groupContainer}>
        ...
        <View style={styles.subPanel}>
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            {/* DATASET CONTEXT */}
            <View style={[styles.datasetBox, { borderColor: theme.colors.border, backgroundColor: theme.colors.card }]}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <Database size={12} color={theme.colors.mutedForeground} />
                    <Text style={{ fontSize: 9, fontWeight: '900', color: theme.colors.mutedForeground }}>CURRENT DATASET</Text>
                </View>
                <Text style={{ fontSize: 13, fontWeight: 'bold' }}>Sine Regression</Text>
                <Text style={{ fontSize: 11, color: theme.colors.mutedForeground, marginTop: 4 }}>
                    A network using only linear math can only draw straight lines, making it mathematically impossible to fit a wavy sine curve. Non-linear activation functions give the network the flexibility to "bend" its predictions to match the data. 
                    {'\n\n'}
                    Without them, no matter how deep the architecture is, the loss landscape will always collapse back into a simple, flat bowl.
                </Text>
            </View>

            {/* NETWORK ARCHITECTURE VISUAL */}
            <View style={[styles.wideGraphBox, { backgroundColor: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.03)', padding: 16, borderRadius: 8, opacity: isLoading ? 0.5 : 1 }]}>
              <NetworkArchitecture
                  inputs={1}
                  depth={depth}
                  width={width}
            ...
            {hasSeenDeepTanh && (
              <Animated.View style={[styles.successBox, successAnimatedStyle, { borderColor: successColor, backgroundColor: successBgColor }]}>
                <Text style={[styles.successText, { color: successColor }]}>
                  ...
                  {'\n\n'}
                  • <Text style={{fontWeight: 'bold'}}>Linear:</Text> Stays a smooth bowl, regardless of depth.
                  {'\n'}
                  • <Text style={{fontWeight: 'bold'}}>ReLU:</Text> Shatters into sharp, piecewise-linear creases.
                  {'\n'}
                  • <Text style={{fontWeight: 'bold'}}>Tanh:</Text> Melts into smooth, curving waves.
                </Text>
              </Animated.View>
            )}
            ...
            <View style={styles.controlGroup}>
              <Text style={[styles.controlLabel, {color: theme.colors.mutedForeground, marginBottom: 4}]}>SELECT ACTIVATION FUNCTION</Text>
              <View style={{ flexDirection: 'row', gap: 8 }}>
                {['Linear', 'ReLU', 'Tanh'].map((act) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.pillText, { color: isActive ? theme.colors.background : theme.colors.mutedForeground }]}>
                        {act}
                      </Text>
                    </TouchableOpacity>
                  );
            ...
            <View style={[styles.controlGroup, { marginTop: 24 }]}>
              <View style={styles.sliderGroup}>
                <Text style={[styles.controlLabel, {color: theme.colors.mutedForeground}]}>NETWORK DEPTH (LAYERS): {depth}</Text>
                <Slider 
                  style={{ flex: 1, height: 40, marginTop: 4 }}
                  value={depth} min={1} max={5} step={1} onValueChange={setDepth} 
                  disabled={isLoading}
              ...
              <View style={styles.sliderGroup}>
                <Text style={[styles.controlLabel, {color: theme.colors.mutedForeground}]}>LAYER WIDTH (NEURONS): {width}</Text>
                <Slider 
                  style={{ flex: 1, height: 40, marginTop: 4 }}
                  value={width} min={1} max={5} step={1} onValueChange={setWidth} 
                  disabled={isLoading}
                  minimumTrackTintColor={theme.colors.accent} thumbTintColor={theme.colors.foreground}
                />
              </View>
        ...
        <View style={[styles.subPanel, { flex: 1.4, backgroundColor: 'rgba(0,0,0,0.05)', position: 'relative' }]}>
          <View style={[styles.subHeader, {borderBottomColor: theme.colors.border, zIndex: 10}]}>
            ...
            <View style={styles.headerControls}>
              ...
              <TouchableOpacity onPress={() => {
                ...
                setDepth(1);
                setWidth(1); 
                setHasSeenDeepLinear(false); setHasSeenDeepReLU(false); setHasSeenDeepTanh(false);
          ...
          {isLoading && (
             <View style={[StyleSheet.absoluteFill, styles.loadingOverlay]}>
                 ...
                 <Text style={{ marginTop: 12, fontWeight: 'bold', color: '#fff' }}>Generating Topology...</Text>
             </View>
          )}
...
const styles = StyleSheet.create({
  ...
  datasetBox: { padding: 12, borderWidth: 1, borderRadius: 8, marginBottom: 20 },
  wideGraphBox: { width: '100%', minHeight: 120, justifyContent: 'center', marginBottom: 20 },
  toggleBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 6, paddingHorizontal: 10, borderRadius: 6, borderWidth: 1 },
