...
import { Network, Layers, ZoomIn, ZoomOut, RefreshCw, Database } from "lucide-react-native";
  ...
  useEffect(() => {
    if (onTaskUpdate) {
      if (hasSeenSmoothing) {
        ...
        if (width >= 3 && depth <= 2) {
          onTaskUpdate(false, "Adding Width smooths the landscape. Increase Depth first to fold the space and create chaos.", true);
        } else {
          onTaskUpdate(false, null);
        }
      } else {
        onTaskUpdate(false, "Now, increase the width. Notice how over-parameterising the network smooths the sweeping valleys out!", true);
      }
    }
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <View style={[styles.datasetBox, { borderColor: theme.colors.border, backgroundColor: theme.colors.card }]}>
               ...
               <Text style={{ fontSize: 11, color: theme.colors.mutedForeground, marginTop: 4 }}>
                 A simple 1-layer network cannot bend to fit a wavy sine curve. As we add hidden layers, the network gains the ability to "fold" the space, creating sweeping periodic valleys in the landscape. 
                 {'\n\n'}
                 <Text style={{ fontStyle: 'italic' }}>
                   (Think of these folds as the network's "flexibility" to bend its predictions—you will learn exactly how this works later!)
                 </Text>
               </Text>
            </View>
            ...
            {/* COMPLEXITY METRIC */}
            <View style={[styles.metricStrip, { backgroundColor: isDark ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.03)', marginBottom: 16 }]}>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <Text style={[styles.metricLabel, { color: theme.colors.mutedForeground }]}>LANDSCAPE COMPLEXITY</Text>
                  <Text style={{ fontSize: 12, fontWeight: '900', color: complexityScore > 80 ? '#ef4444' : theme.colors.foreground }}>
                    {complexityScore}%
                  </Text>
                </View>
                <Text style={{ fontSize: 20, fontWeight: 'bold', color: theme.colors.foreground, marginBottom: 12 }}>
                  {complexityLabel}
                </Text>
                <View style={{ height: 6, width: '100%', backgroundColor: theme.colors.border, borderRadius: 3, overflow: 'hidden' }}>
                  <View style={{ 
                    ...
                    backgroundColor: complexityScore > 80 ? '#ef4444' : theme.colors.accent 
                  }} />
                </View>
              </View>
            </View>
            ...
            {hasSeenSmoothing && (
              <Animated.View style={[styles.successBox, successAnimatedStyle, { borderColor: successColor, backgroundColor: successBgColor }]}>
                <Text style={[styles.successText, { color: successColor }]}>
                  ...
                  {'\n\n'}
                  Adding Depth creates chaotic folds and local minimums, but making the layers Wider (over-parameterisation) smooths the landscape out, making it easier for Optimisers to find the bottom.
                </Text>
              </Animated.View>
            )}
          ...
          <View style={[styles.subHeader, {borderBottomColor: theme.colors.border, zIndex: 10}]}>
            ...
            <View style={styles.headerControls}>
              ...
              <TouchableOpacity onPress={() => {
                setDepth(1); setWidth(1); 
                setHasSeenChaos(false); setHasSeenSmoothing(false);
