...
export default function NetworkVis({ inputCount, depth, width, activation, outputCount }: NetworkVisProps) {
  ...
  const targetLayout = useMemo(() => {
    ...
    for (let i = 0; i < inputCount; i++) {
      nodes.push({ id: `in-${i}`, x: getX(0), y: getY(i, inputCount), type: 'input', label: '', opacity: 1 });
    }

    // Hidden Layers
    for (let l = 1; l <= depth; l++) {
      for (let i = 0; i < width; i++) {
        nodes.push({ id: `h-${l}-${i}`, x: getX(l), y: getY(i, width), type: 'hidden', label: '', opacity: 1 });
      }
    }
  ...
  const displayEdges = useMemo(() => {
    ...
    if (hiddenLayers.length > 0) {
      inputNodes.forEach(src => {
        hiddenLayers[0].forEach(tgt => edges.push({ x1: src.x, y1: src.y, x2: tgt.x, y2: tgt.y }));
      });
    } else {
        // Direct connection if no hidden layers
        inputNodes.forEach(src => {
            outputNodes.forEach(tgt => edges.push({ x1: src.x, y1: src.y, x2: tgt.x, y2: tgt.y }));
        });
    }
    ...
    for (let i = 0; i < hiddenLayers.length - 1; i++) {
      hiddenLayers[i].forEach(src => {
        hiddenLayers[i+1].forEach(tgt => edges.push({ x1: src.x, y1: src.y, x2: tgt.x, y2: tgt.y }));
      });
    }
  ...
  return (
    <View style={styles.container} onLayout={onLayout}>
      ...
      <View style={styles.labelContainer}>
         <Text style={styles.infoText}>{activation} • {depth} Hidden • {width} Width</Text>
      </View>
    </View>
  );
}
