...
function LossHeatmap({ onResult }: { onResult: (isCorrect: boolean) => void }) {
  const [red, setRed] = useState(0);
  const [blue, setBlue] = useState(0);
  const [gameWon, setGameWon] = useState(false);
  const [isHeld, setIsHeld] = useState(false); // Track if mouse is held down
  ...
  const currentLoss = Math.sqrt(
    Math.pow(red - 0.5, 2) + Math.pow(blue - 0.5, 2),
  );
  ...
  useEffect(() => {
    if (
      !gameWon &&
      currentGridX === TARGET_INDEX &&
      currentGridY === TARGET_INDEX &&
      !isHeld
    ) {
      setGameWon(true);
  ...
  }, [currentGridX, currentGridY, gameWon, scale, visited, onResult, isHeld]);
    ...
    <View>
      <View style={styles.headerBlock}>
        ...
        <ThemedText style={styles.subText}>
          Adjust the sliders to find the combination of red and blue that
          minimises the loss (a.k.a makes purple).
          {'\n'}
          The map is hidden until you explore it. (Hint: red = high loss, purple
          = low loss)
        </ThemedText>
      </View>
          ...
          <View style={styles.sliderGroup}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Slider
                ...
                minimumTrackTintColor={gameWon ? '#666' : '#1e00ff'}
                thumbTintColor={gameWon ? '#888' : '#fff'}
                disabled={gameWon}
              />
              <ThemedText
                style={{ marginLeft: 10, minWidth: 35, fontSize: 12 }}
              >
                {(blue * 100).toFixed(0)}%
              </ThemedText>
            </View>
          ...
          <View style={styles.sliderGroup}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              <Slider
                ...
                minimumTrackTintColor={gameWon ? '#666' : '#ff4d4d'}
                thumbTintColor={gameWon ? '#888' : '#fff'}
                disabled={gameWon}
              />
              <ThemedText
      ...
      {gameWon ? (
        <Animated.View style={[styles.successBox, animatedStyle]}>
          <ThemedText style={styles.successText}>
            ...
            {'\n'}
            Hence, the loss is minimised at this point, and becomes larger the
            further away you move.
          </ThemedText>
        </Animated.View>
      ) : (
