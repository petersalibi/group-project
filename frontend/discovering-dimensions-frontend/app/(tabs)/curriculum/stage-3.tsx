  ...
  return (
    <>
      ...
      <ThemedText type='text'>
        ...
        activations. And like linear models, we have a loss function, often a
        sum-of-squares / MSE loss, that, for a given set of predictions and
        labels, determines the loss of our network.
        {'\n'}
        {'\n'}
        The idea is <ThemedText type='textItalic'>relatively</ThemedText>{' '}
        straightforward, but often hard to implement in real software:
        <OrderedList>
          <ThemedText type='text'>
            Perform the <ThemedText type='textBold'>forward pass</ThemedText>{' '}
            &ndash; that is, run the network through the training data to get
            the predictions <Math exp='\hat{\textbf{y}}' />.
          </ThemedText>
          <ThemedText type='text'>
            Compute the loss{' '}
          ...
          <ThemedText type='text'>
            Compute the <ThemedText type='textBold'>gradient vector</ThemedText>{' '}
            of all weights with respect to the loss <Math exp='L' />. So for
            each weight <Math exp='w' />, compute <Math exp='\frac{dL}{dw}' />{' '}
            &ndash; how does the loss change with respect to the value of the
            weight? This is just saying, &quot;In what direction should we move
            this weight so that the loss increases?&quot; Since we want the loss
            to decrease, if we take the negative of this, we can hopefully make
            a change in the direction of <Math exp='\frac{dL}{dw}' /> that
            reduces the loss further.
          </ThemedText>
          <ThemedText type='text'>
            ...
            rate. We make this a small number, e.g. 0.05, so that we don&apos;t
            go <ThemedText type='textItalic'>too</ThemedText> far in the
            direction of loss reduction or we might actually miss a minimum
            (imagine jumping across the valley in the loss landscape rather than
            falling to the middle &ndash; too far a gap).
          </ThemedText>
        </OrderedList>
        {'\n'}
        {'\n'}
      ...
      <ThemedText type='text'>
        ...
        vector as the{' '}
        <ThemedText type='textBold'>average gradient vector</ThemedText> of all
        the training examples, or, given like this:
        <Math
          block={true}
        ...
        <ThemedText type='textBold'>SGD</ThemedText>):
        <Math
          block={true}
      ...
      <ThemedText type='subheading'>
        How different optimisers influence movement across the landscape
      </ThemedText>
      <ThemedText type='text'>
        We&apos;ve already seen how gradient descent (GD) and stochastic
        gradient descent (SGD) will move along the loss landscape &ndash; a
        straight path versus a bendy, twisting one, but both converging at the
        minimum. And in general so long as our optimising algorithm will
        converge the weights to reach a nontrivial loss minimum, it is
        technically an optimiser. But what about specific{' '}
        <ThemedText type='textItalic'>types</ThemedText> of optimisers?
        {'\n'}
        {'\n'}
        To understand this, we can look at each optimisation algorithm in turn
        and observe the behaviour of how the weights change and adapt. Then we
        can attempt to gauge how this would appear visually with a ball moving
        throughout the 3D loss landscape. Of course, in reality our moving
        &quot;ball&quot; would have as many dimensions as there are weights, as
        would the landscape itself, and so visualising loss would be quite
        tricky! But as we will see later, there are ways to get around the
        higher dimensionality. For now, just assume we are using a network with
        only two modifiable parameters.
      </ThemedText>
      <ThemedText type='subsubheading'>
        Optimisation addition 3: variable learning rate
      </ThemedText>
      <ThemedText type='text'>
        This is just when we make the learning rate &ndash; the scalar parameter
        that we apply to ensure that the gradient magnitude is not too high
        &ndash; variable in <Math exp='t' />, the number of iterations. This
        means...
      </ThemedText>
      <ThemedText type='subheading'>
        More complex loss landscape visualisation methods
      </ThemedText>
      <ThemedText type='subsubheading'>
      ...
      <ThemedText type='text'>
        ...
        <ThemedText type='textBold'>principal component analysis</ThemedText> (
        <ThemedText type='textBold'>PCA</ThemedText>) comes in. This is a way to
      ...
      <ThemedText type='text'>
        ...
        <OrderedList>
          ...
          <ThemedText type='text'>
            Perform PCA on <Math exp='\mathbf{M}' />. In effect, this means
            constructing two matrices, <Math exp='\mathbf{Z}' /> and{' '}
            <Math exp='\mathbf{W}' />, which, when matrix-multiplied together,
            return the matrix <Math exp='\mathbf{M} = \mathbf{ZW}' />. The exact
            specifics of finding these two matrices relies on eigenvalues and
            eigenvectors and a process called singular value decomposition,
            which is what will be learnt and fully covered in the{' '}
            <Link
              href='https://warwick.ac.uk/fac/sci/dcs/teaching/modules/cs342/'
              target='_blank'
            >
      ...
      <ThemedText type='subsubsubheading'>Advantages</ThemedText>
      <UnorderedList>
        <ThemedText type='text'>
      ...
      <ThemedText type='subsubsubheading'>Disadvantages</ThemedText>
      <UnorderedList>
        <ThemedText type='text'>
      ...
      <ThemedText type='text'>
        ...
        location <Math exp='\textbf{x}\hspace{-0.1em} \in \mathbb{R}^d' />,
        which for our purposes would represent a vector of network weights that
        produces a local loss minimum, and we have random vectors{' '}
        <Math exp='\textbf{p}' /> and <Math exp='\textbf{q}' />, then we can
        &quot;move&quot; along this plane using the formula:
        <Math
          block={true}
          exp='f(\alpha, \beta) = L(\textbf{x} + \alpha \textbf{p} + \beta \textbf{q})'
        />
        where <Math exp='L' /> is the loss function (since we need to establish
        the loss to plot the landscape) and we can imagine <Math exp='\alpha' />{' '}
        and <Math exp='\beta' /> as the x- and y-axes of a plane. Note that in
        order to ensure the axes are of the same scale, we need to normalise{' '}
        <Math exp='\textbf{p}' /> and <Math exp='\textbf{q}' /> so that their
        Euclidean norms are equal to 1, or else it would create visual
        distortions. Then, once we&apos;ve moved some direction in each random
        vector away from the starting point <Math exp='\textbf{x}' />, this
        represents a possible space of parameters for the neural network &ndash;
        so we can load in those parameters and compute the loss on the training
        data.
        {'\n'}
        {'\n'}
        Once we have iterated over enough values for <Math exp='\alpha' /> and{' '}
        <Math exp='\beta' /> and recorded the loss for each to get triples of
        points, we can then plot a 3D loss surface, with <Math exp='\alpha' />{' '}
        and <Math exp='\beta' /> as the x- and y-axes and the loss as the
      ...
      <Figure
        img={require('@/assets/images/curriculum/stage-3/loss-contour.webp')}
        width={700}
        aspectRatio={4.4}
        caption='The loss landscape, portrayed as a contour map. Each coloured ring represents a magnitude of loss.'
      />
      <ThemedText type='subsubsubheading'>Summary of core steps</ThemedText>
      <ThemedText type='text'>
        <OrderedList>
          <ThemedText type='text'>
      ...
      <UnorderedList>
        ...
        <ThemedText type='text'>
          <ThemedText type='textBold'>
            Magnitude and shape of loss dependent on network architecture
          </ThemedText>{' '}
          &ndash; vulnerable to scale invariance, making comparison impossible
        </ThemedText>
      </UnorderedList>
      <ThemedText type='subsubheading'>Filter(-wise) normalisation</ThemedText>
      <ThemedText type='text'>
        ...
        subtracts the mean and divides by the standard deviation + a small
        offset. Therefore we could have an infinite number of networks which
        differ by weight values, but are guaranteed to have the same outputs due
        to scale invariance&apos;s effect and the relative magnitudes of the
        weight values being the same. Yet these networks if their loss
        landscapes were plotted with random directions would have completely
        ...
        <OrderedList>
          <ThemedText type='text'>
            ...
            -th neural network <Math exp='\textbf{n}_{\hspace{-0.1em}k}' />
            &apos;s parameters <Math exp='\theta_k' />.
          </ThemedText>
          <ThemedText type='text'>
            For every filter (or weight) in <Math exp='d_l' />, Replace the
            values of the filter with the ratio between the{' '}
            <ThemedText type='textBold'>Frobenius norm</ThemedText> of the
            corresponding filter in <Math exp='\theta_k' /> and the
            filter&apos;s Frobenius norm, or, in other words:
            <Math
              block={true}
              exp='F_k(d_{l,i,j}) = d_{l,i,j} \times \frac{||\theta_{k,i,j}||_F}{||d_{l,i,j}||_F}.'
            />
          </ThemedText>
          <ThemedText type='text'>
      ...
      <ThemedText type='subheading'>
        Moving beyond loss landscapes &ndash; visualising network training
      </ThemedText>
      <ThemedText type='subsubheading'>
      ...
      <ThemedText type='text'>
        ...
        <OrderedList>
          ...
          <ThemedText type='text'>
            ...
            generated by an{' '}
            <ThemedText type='textBold'>instability function</ThemedText> that
            takes in the history of cluster assignments to that point and
            returns a real number &ndash; this is usually the &quot;jump&quot;
            function, or how many times the cluster changes throughout the
            history, averaged out over many iterations.
          </ThemedText>
          <ThemedText type='text'>
      ...
      <Figure
        img={require('@/assets/images/curriculum/stage-3/instability-map.webp')}
        width={500}
        aspectRatio={1.25}
      ...
      />
      <Figure
        img={require('@/assets/images/curriculum/stage-3/instability-histogram.webp')}
        width={500}
        aspectRatio={1.5}
      ...
      <ThemedText type='text'>
        ...
        <OrderedList>
          ...
          <ThemedText type='text'>
            ...
            corresponding instability value{' '}
            <Math exp='v(\textbf{x}\hspace{-0.2em})' />.
          </ThemedText>
          <ThemedText type='text'>
      ...
      <UnorderedList>
        ...
        <ThemedText type='text'>
          Intuitive and useful for identifying hard points in
